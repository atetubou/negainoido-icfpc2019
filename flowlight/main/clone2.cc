...
int main(int argc, char** argv) {
  ...
  set<int> required_vertices = collect_required_vertices(ai.board);
  vector<pair<int, int> > board_vertices;
  for (const auto index: required_vertices) {
    int h = to_position(index).first;
    int w = to_position(index).second;
    board_vertices.push_back(make_pair(h, w));
  }
  required_vertices.insert(start_index);
  const int n = sqrt(ai.board.size() * ai.board[0].size() - ai.get_block_count()) * 5;
  
  // vector<int> tour = compute_tour(start_index, required_vertices, original_gd);
  vector<pair<int, int>> tikuta = tikutaOrder(ai, n, board_vertices);
  vector<int> tour;
  for (const auto &p: tikuta) {
    tour.push_back(to_index(p.first, p.second));
  }
  ...
  if (clone_collect_moves.empty() || x_vertices.empty() || tour.size() == 1) {
    ...
    for (size_t i = 0; i < subtours.size(); i++) {
      ...
      res += enjoy_tour(start_index, subtours[i], visited_indices, wrapped_indices, ai.board, original_gd);
    }
  }
  cerr << res.size() << endl;
  cout << res << endl;
